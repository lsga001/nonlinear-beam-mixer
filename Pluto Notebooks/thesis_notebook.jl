...
# ╔═╡ 203509fc-bfd1-42d3-b1ba-567e3d8d4082
begin
	using Revise
end

# ╔═╡ 723bbeb6-4275-11f1-97b8-67bcd3a008ee
begin
	using Optikos
	using CairoMakie
...
# ╔═╡ d82498f4-9a1e-4d77-8d1f-a3a9bb4d1083
md"""
## Packages
"""
...
begin
	"""
    disk_schell_mixer(grid, source, system1, system2, system3, mixer;
...
print("Coherence length: ", Lc/aperture)
...
begin
	"""
    beam_wander_mixer(grid, beam1, beam2, radius, system1, system2, system3, mixer;
                      n_realizations=500) -> Matrix{ComplexF64}

Monte Carlo simulation of a two-arm nonlinear mixing experiment using
two beam wandered beams with the same distribution of centroid points (at the start).
...
function beam_wander_mixer(
  ...
  is_same_source :: Bool
		   ...
		   Vector{Tuple{Float64, Float64}}, Vector{Tuple{Float64, Float64}}}
  @assert n_realizations > 0
  ...
  centers_1 = Vector{Tuple{Float64,Float64}}(undef, n_realizations)
  centers_2 = Vector{Tuple{Float64,Float64}}(undef, n_realizations)

  x10, y10 = _sample_disk(radius)
  x20, y20 = is_same_source ? (x10, y10) : _sample_disk(radius)
	  
  centers_1[1] = (x10, y10)
  centers_2[1] = (x20, y20)
		
  b1 = @set beam1.center = (x10, y10)
  field1 = evaluate(grid, b1)

  b2 = @set beam2.center = (x20, y20)
  field2 = evaluate(grid, b2)
  ...
  W1 = zeros(ComplexF64, grid1.Ny, grid1.Nx)
  W2 = zeros(ComplexF64, grid2.Ny, grid2.Nx)
  W3 = zeros(ComplexF64, grid3.Ny, grid3.Nx)

  # Why can't I use threads? Data races...
  for k in 2:n_realizations
    x10, y10 = _sample_disk(radius)
	x20, y20 = is_same_source ? (x10, y10) : _sample_disk(radius)
  ...
  field1 = ScalarField(W1 ./ n_realizations, grid1, λ1)
  field2 = ScalarField(W2 ./ n_realizations, grid2, λ2)
  field3 = ScalarField(W3 ./ n_realizations, grid3, λ_out)

  return field1, field2, field3, centers_1, centers_2
end
...
begin
	...
	if beamtype2 == "LG"
		...
		beam22 = LGBeam(w02, λ2, p22, l22);
	elseif beamtype2 == "HG"
		beam21 = HGBeam(w02, λ2, p21, l21);
		beam22 = HGBeam(w02, λ2, p22, l22);
	end
...
function my_fig(field; beam_centers=missing, source_radius=missing, plottype=missing, units=missing, save_path=missing)

	local plot_units = (units !== missing) ? units : :m
	local scale = 	(plot_units == :mm) ? 1e3 :
					(plot_units == :μm) ? 1e6 :
					(plot_units == :nm) ? 1e9 : 1e0
	local units_str = 	(plot_units == :mm) ? "mm" :
						(plot_units == :μm) ? "μm" :
						(plot_units == :nm) ? "nm" : "m"
	...
	local ax = Axis(
		...
		xtickformat = "{:.1f}",
        ytickformat = "{:.1f}",
		xlabel = rich(rich("x", font=:italic), " / "*units_str),
		ylabel = rich(rich("y", font=:italic), " / "*units_str),
	);
	...
	if (beam_centers !== missing)
		local scatter_points = Point2f.(first.(beam_centers), last.(beam_centers));
		scatter!(
			ax, 
			scale * scatter_points, 
			color=(:orange,0.1), 
			markersize=4, 
			strokewidth=1, 
			strokecolor=(:red,0.1));
	...
	if (source_radius !== missing)
		# Circle parameters
		local r = scale * source_radius;
		local θ = range(0, 2π, length=300);
		local xc = r * cos.(θ);
		local yc = r * sin.(θ);
...
begin
	my_fig(
		...
		plottype=:intensity, 
		units=:mm,
		beam_centers=points1,
		source_radius=source_radius1,
		save_path="thesis_notebook_figures/$beamtype1$p11$l11-$beamtype1$p12$l12/beam-wander-1.pdf"
	)
end
...
begin
	my_fig(
		...
		plottype=:intensity, 
		units=:mm,
		#beam_centers=points2,
		#source_radius=source_radius1,
		save_path="thesis_notebook_figures/$beamtype1$p11$l11-$beamtype1$p12$l12/beam-wander-2.pdf"
	)
end
...
begin
	my_fig(
		...
		plottype=:abs, 
		units=:mm,
		save_path="thesis_notebook_figures/$beamtype1$p11$l11-$beamtype1$p12$l12/beam-wander-3.pdf"
	)
end
...
function my_phase(field; source_radius=missing, plottype=missing, units=missing, save_path=missing)
...
Optikos = "94154c6d-4466-4fcb-80cd-488809413af4"
...
git-tree-sha1 = "4f8627e9b7c49c1cc1e5e120faf33e4528a7c93f"
...
[[deps.Optikos]]
deps = ["CairoMakie", "FFTW"]
path = "/home/leo/Contributions/Julia/Optikos"
uuid = "94154c6d-4466-4fcb-80cd-488809413af4"
version = "0.1.0"
