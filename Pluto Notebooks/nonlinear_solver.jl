...
# ╔═╡ 1a615f5d-6343-46cf-98d8-ba13c2c1c393
md"""
We now want to do a rough simulation of the conditions of the experiment.

Firstly, the experiment aims to generate and measure arbitrary beams that exhibit the behavior required from a mutual coherence function.

To do this we imagine an optical setup that takes two incoming vector beams of frequencies ``\omega_1`` and ``\omega_2``, and mixes them using a nonlinear crystal to produce a new beam of frequency ``\omega_3=\omega_1+\omega_2``.

In practice the beam coming out of the crystal will be a linear sum of the two input beams and the desired beam at frequency ``\omega_3``. However we can use a filter to let through the desired beam.

The reason for the experimental setup is that we know that a mutual coherence function has the form
```math
\Gamma(\vec{r}_1,\vec{r}_2,\tau) = \left<U^*(\vec{r}_1,t) U(\vec{r}_2,t+\tau)\right>,
```
where ``\left<\cdot\right>`` denotes a ensemble average over many realizations of the random function (Saleh, Ch. 12.1).

We know that the result of passing two beams through a nonlinear crystal is that we get a new beam that is the scaled product of both.
To wit,
```math
U_3(\vec{r}) = U_1(\vec{r})U_2(\vec{r}).
```
Notice however how we are unable to _choose_ the position vectors independently after the fact. That is, we can't mix different positions of the input beams while the nonlinear mixing is taking place---it will be performed in-place.

To simulate a sort of cross-position mixing required for the mutual coherence function, we can try to do it with the SLM _before_ it reaches the nonlinear crystal.
We note this is very inefficient, but for now is the only method that comes to mind.
"""

# ╔═╡ e0e37aed-2fc7-45e7-81f8-afc6a564ffda
md"""
We will simulate an ensemble of spherical point sources at a reference origin plane, randomly displaced within a gaussian profile for both input beams.
We will propagate each instance of the ensemble, "mix" pairs(?) of instances between both, create an instance of the output beam, and then average the values over all instances created.
"""
...
begin
	...
	signal = Beam(1e-3*circular_mask, 1.06e-6);
	pump = Beam(1e5*exp.(-(xv.^2 .+ yv'.^2)/w0^2), 1.06e-6);
...
function beam_generation(xv, yv, z, position_pair, input_wavelength_pair; source_type = "spherical")
	...
	U₁_wavelength = input_wavelength_pair[1];
	U₂_wavelength = input_wavelength_pair[2];
...
begin
	heatmap(xv, yv, abs2.(U₁.amplitude), aspect_ratio=:equal, size=(600,550), right_margin=15Plots.mm, title="Input beam 1 just before crystal")
	scatter!((xₒ₁, yₒ₁))
end

# ╔═╡ 9fa04509-0d35-4abd-b26b-ae554881582a
begin
	heatmap(xv, yv, abs2.(U₂.amplitude), aspect_ratio=:equal, size=(600,550), right_margin=15Plots.mm, title="Input beam 2 just before crystal")
	scatter!((xₒ₂, yₒ₂))
end
...
manifest_format = "2.0"
project_hash = "ffb9435f885e2570bbece54baaa5714efb3998d9"
...
git-tree-sha1 = "033e77ea6d39153118bda0f560d03d05287aa5dd"
